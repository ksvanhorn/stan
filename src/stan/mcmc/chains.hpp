...
namespace stan {  
  ...
  namespace mcmc {
    ...
    private:
      ...
      static Eigen::VectorXd quantiles(const Eigen::VectorXd& x, const Eigen::VectorXd& probs) {
        using boost::accumulators::accumulator_set;
        using boost::accumulators::left;
        using boost::accumulators::quantile_probability;
        using boost::accumulators::right;
       ...
       * @return 
       */
      double effective_sample_size(const Eigen::Matrix<Eigen::VectorXd, Eigen::Dynamic, 1> &samples) const {
       ...
       * @return 
       */
      static double split_potential_scale_reduction(const Eigen::Matrix<Eigen::VectorXd, Eigen::Dynamic, 1> &samples) {
        int chains = samples.size();
        int n_samples = samples(0).size();
        for (int chain = 1; chain < chains; chain++) {
          n_samples = std::min(n_samples, int(samples(chain).size()));
    ...
    public:
      ...
      double quantile(const int chain, const std::string& name, const double prob) const {
      ...
      Eigen::VectorXd quantiles(const int chain, const int index, const Eigen::VectorXd& probs) const {
      ...
      Eigen::VectorXd quantiles(const int chain, 
                                const std::string& name, const Eigen::VectorXd& probs) const {
      ...
      Eigen::Vector2d central_interval(const int chain, const int index, const double prob) const {
      ...
      Eigen::Vector2d central_interval(const int index, const double prob) const {
        double low_prob = (1-prob)/2;
        double high_prob = 1-low_prob;
      ...
      Eigen::Vector2d central_interval(const int chain, 
                                       const std::string& name, const double prob) const {
        return central_interval(chain, index(name), prob);
      }

      Eigen::Vector2d central_interval(const std::string& name, const double prob) const {
      ...
      Eigen::VectorXd autocovariance(const int chain, const std::string& name) const {
